...
export const kanbanMethodologyContentRu: MethodologyContent = {
  ...
  overview: {
    summary:
      "Потоковый метод, который делает работу видимой, ограничивает WIP и улучшает поставку через вытягивающие правила и наблюдаемое поведение потока",
    entries: [
      {
        id: "whatIs",
        title: "Что такое Kanban",
        description:
          "Kanban — это потоковый метод управления работой через визуализацию, явные правила движения задач и ограничения незавершенной работы. В программной инженерии он особенно связан с непрерывной поставкой, средами с тяжелым сопровождением и ситуациями, где входящая работа изменчива и плохо укладывается в фиксированные итерации"
      },
      {
        id: "coreIdea",
        title: "Ключевая идея",
        description:
          "Вместо организации поставки вокруг спринтов фиксированной длины Kanban строит работу как непрерывный поток. Новая работа обычно берется только тогда, когда освобождается емкость команды. Поэтому подход особенно уместен там, где приоритеты часто меняются и команде нужно реагировать непрерывно, а не ждать границы следующей итерации"
      },
      {
        id: "whyItMatters",
        title: "Почему это важно",
        description:
          "Kanban дает командам легкий, но дисциплинированный способ видеть узкие места, снижать перегрузку, стабилизировать пропускную способность и сокращать время прохождения задачи. В модели проекта это самый явный представитель процесса с непрерывным потоком и WIP-ограничениями"
      }
    ],
    signalTags: [
      { id: "continuousFlow", label: "Непрерывный поток" },
      { id: "wipLimits", label: "WIP-лимиты" },
      { id: "pullBasedDelivery", label: "Вытягивающий принцип работы" },
      { id: "explicitWorkflowPolicies", label: "Явные правила потока" }
    ]
  },
  quickFit: {
    title: "Быстрый ориентир",
    summary:
      "Лучше всего подходит для непрерывного сервисного потока, работы поддержки и меняющихся приоритетов, где визуальный контроль и WIP-дисциплина важнее спринтового ритма"
  },
  firstSteps: {
    title: "С чего начать применение Kanban",
    intro:
      "Сначала опишите реальный рабочий поток, а затем сделайте его видимым и управляемым, прежде чем пытаться ускорить систему",
    mode: "sequence",
    steps: [
      { id: "visualizeCurrentWorkflow", title: "Визуализировать текущий рабочий поток", description: "Опишите реальные стадии прохождения работы: прием, анализ, разработка, тестирование, релиз. Отталкивайтесь от фактического процесса, а не от идеализированной схемы" },
      { id: "createFirstBoard", title: "Создать первую Kanban-доску", description: "Представьте рабочий поток визуально через колонки и карточки работы. Важно, чтобы доска отражала реальные передачи задач и очереди, а не набор абстрактных подписей" },
      ...
      { id: "trackFlowMetrics", title: "Отслеживать метрики потока", description: "Измеряйте lead time, cycle time, throughput и блокеры. Не превращайте метрики в бюрократию — используйте их, чтобы понимать, где поток ломается" },
      { id: "improveBottlenecksContinuously", title: "Непрерывно улучшать узкие места", description: "Используйте доску и метрики, чтобы находить очереди, блокировки и нестабильные стадии. Улучшайте сначала самые слабые места, а не переделывайте систему целиком", emphasis: "final" }
    ],
    callout: {
      id: "practicalNote",
      label: "Практическая заметка",
      description:
        "Kanban лучше всего работает тогда, когда команда начинает с текущего процесса и эволюционно улучшает его. Подход не требует драматического запуска, но требует дисциплины в визуализации, вытягивающей логике и контроле WIP"
    }
  },
  coreElements: {
    ...
    groups: [
      {
        ...
        label: "Принципы",
        items: [
          { id: "visualizeWorkflow", title: "Визуализировать рабочий поток", description: "Доска делает фактический поток работы видимым, чтобы команда могла замечать прогресс, очереди и блокеры" },
          { id: "limitWorkInProgress", title: "Ограничивать незавершенную работу", description: "WIP-лимиты снижают перегрузку и заставляют завершать работу вместо бесконечного старта новых задач" },
          { id: "manageFlow", title: "Управлять потоком", description: "Команда отслеживает, как работа проходит по системе, особенно задержки, блокировки и нестабильную пропускную способность" },
      ...
      },
      {
        ...
        items: [
          { id: "workflowColumns", title: "Колонки рабочего потока", description: "Колонки отражают реальные состояния процесса, а не абстрактные заглушки. Типичные примеры: анализ, разработка, тестирование, выпуск" },
          ...
          { id: "queuesAndBlockers", title: "Очереди и блокеры", description: "Ожидание и заблокированная работа должны быть видны, потому что скрытые очереди разрушают поток и маскируют реальные проблемы" }
        ]
      },
      {
        id: "wipLimitsAndPullLogic",
        label: "WIP-лимиты и вытягивающая логика",
        items: [
          { id: "pullBasedStartOfWork", title: "Начинать работу по готовности команды", description: "Команда должна начинать новую работу тогда, когда есть реальная емкость, а не продавливать задачи в уже перегруженные стадии" },
          { id: "stageLevelWipControl", title: "Контроль WIP по стадиям", description: "Ограничения нужно ставить там, где вероятна перегрузка. Если стадия переполнена, работа выше по потоку должна остановиться или помочь устранить узкое место" },
      ...
      },
      {
        ...
        items: [
          ...
          { id: "flowEfficiencyAndBottlenecks", title: "Эффективность потока и узкие места", description: "Команда должна наблюдать, где работа ждет, а не только где она активно выполняется" }
        ]
      }
    ]
  },
  teamNeeds: {
    ...
    items: [
      ...
      { id: "sharedPullUnderstanding", text: "Общее понимание вытягивающего принципа управления работой" },
      { id: "blockerTransparency", text: "Прозрачность по блокерам и состояниям ожидания" },
    ...
    callout: {
      ...
      label: "Практическая заметка",
      description:
        "Kanban не требует такой формальной ролевой структуры, как Scrum, но все равно требует поведенческой дисциплины. Без нее доска становится декоративной, а WIP-лимиты — фикцией"
    }
  },
  commonMistakes: {
    ...
    items: [
      ...
      { id: "hideQueuesAndBlockers", text: "Визуализировать задачи, но не показывать очереди и блокеры" },
      { id: "oversizedWorkItems", text: "Делать карточки работы слишком большими или слишком расплывчатыми" },
      { id: "measureWithoutAction", text: "Измерять поток, но ничего не делать с узкими местами" },
      ...
      { id: "mixedWorkWithoutPolicies", text: "Смешивать работу поддержки и стратегические фичи без явных правил" }
    ],
    callout: {
      id: "whyTheseMistakesMatter",
      label: "Почему это важно",
      description:
        "Kanban выглядит простой, поэтому ее легко недоинженерить. Когда команда сохраняет только доску, но игнорирует вытягивающую логику, лимиты и управление узкими местами, она воспроизводит видимость потока без реальных выгод"
    }
  },
  applicability: {
    ...
    goodFit: [
      ...
      { id: "supportOpsMaintenanceFlow", text: "Команда ведет поддержку, операционную работу, сопровождение или поток дефектов" },
      { id: "varyingTaskSizeAndUrgency", text: "Задачи заметно различаются по размеру и срочности" },
      { id: "leadTimeOverIterationCadence", text: "Сокращение времени прохождения задачи важнее фиксированного ритма итераций" },
      { id: "evolutionaryChangeApproach", text: "Организации подходит эволюционный подход к изменениям, а не полная смена процесса" },
      { id: "visibilityOfQueuesAndCapacity", text: "Поставка выигрывает от сильной видимости очередей, блокеров и доступной емкости по стадиям" }
    ],
    weakerFit: [
